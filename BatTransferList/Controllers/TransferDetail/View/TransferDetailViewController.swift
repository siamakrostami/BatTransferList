...
// MARK: - TransferDetailViewController
...
extension TransferDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: PersonalTVC.identifier, for: indexPath) as? PersonalTVC else {
                return .init()
            }
            ...
            return cell
        case 1:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CardTVC.identifier, for: indexPath) as? CardTVC else {
                return .init()
            }
            cell.selectionStyle = .none
            cell.configureCell(model: self.viewModel.currentModel)
            return cell
        case 2:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: TransferTVC.identifier, for: indexPath) as? TransferTVC else {
                return .init()
            }
            cell.selectionStyle = .none
            cell.configureCell(model: self.viewModel.currentModel)
            return cell
        default:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: NoteTVC.identifier, for: indexPath) as? NoteTVC else {
                return .init()
            }
            cell.selectionStyle = .none
            cell.configureCell(model: self.viewModel.currentModel)
